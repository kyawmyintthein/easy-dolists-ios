...
- (void)viewWillLayoutSubviews
{
    ...
    
    {
        ...
        CGFloat y = (_labelTitle.text == nil) ? (kCircleHeight - 20.0f) : 74.0f;
        y += _subTitleHeight + 14.0f;
        for (UITextView *textView in _inputs)
        {
            textView.frame = CGRectMake(12.0f, y, _windowWidth - 24.0f, 80.0f);
        ...
        for (SCLButton *btn in _buttons)
        {
            btn.frame = CGRectMake(12.0f,(y+50), btn.frame.size.width, btn.frame.size.height);
            btn.layer.cornerRadius = 3.0f;
            y += btn.frame.size.height + 10.0f;
        }
    }
...
- (UITextView *)addTextView:(NSString *)title
{
    [self addObservers];
...
- (void)addCustomTextField:(UITextField *)textField
{
    ...
    CGRect frameRect2 = textField.frame;
    frameRect2.size.height = 80.0f;
    textField.frame = frameRect2;
